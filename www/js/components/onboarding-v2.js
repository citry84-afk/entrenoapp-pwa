...
const onboardingSteps = [
    ...
    },
    {
        ...
        subtitle: 'Revisa y confirma antes de generar'
    }
];
...
window.initOnboardingPage = function() {
    ...
    try {
        ...
        // Comprobar si viene desde "Crear nuevo plan" con tipo pre-seleccionado
...
// Aplicar preset de tipo de plan (desde Gestión de Planes)
function applyOnboardingPreset() {
    try {
        const preset = localStorage.getItem('entrenoapp_onboarding_preset');
        if (!preset) return;
        const parsed = JSON.parse(preset);
        const planType = parsed.plan_type;
        localStorage.removeItem('entrenoapp_onboarding_preset');
        if (!planType || !['running', 'functional', 'gym'].includes(planType)) return;
        onboardingState.userData.plan_type = planType;
        onboardingState.currentStep = 2; // Saltar welcome y plan_type, ir a experience
        debugLog('PRESET_APPLIED', 'Tipo pre-seleccionado', { planType });
    } catch (e) {
        debugLog('PRESET_ERROR', 'Error aplicando preset', e);
    }
}
            ...
            <div class="step-navigation">
                ...
                ${currentStepData.type !== 'welcome' ? `
                    <button id="next-btn" class="glass-button glass-button-primary" ${!canProceedToNext() ? 'disabled' : ''}>
                        ${onboardingState.currentStep === onboardingSteps.length - 1 ? 'Crear mi plan' : 'Siguiente →'}
                    </button>
                ` : `
...
function renderStepContent(stepData) {
    switch (stepData.type) {
        ...
        case 'summary':
            return renderSummaryStep(stepData);
        default:
...
const PLAN_TYPE_LABELS = { running: 'Plan de Carrera', functional: 'Entrenamiento Funcional', gym: 'Gimnasio' };
const EXPERIENCE_LABELS = { beginner: 'Principiante', intermediate: 'Intermedio', advanced: 'Avanzado' };
const SESSION_LABELS = { short: '30-45 min', medium: '45-60 min', long: '60-90 min' };
const FREQUENCY_LABELS = { low: '3 días/semana', medium: '4-5 días/semana', high: '6+ días/semana' };
const EQUIPMENT_LABELS = { gym_access: 'Gimnasio completo', home_gym: 'Gimnasio casero', basic_equipment: 'Equipamiento básico', bodyweight_only: 'Solo peso corporal', outdoor_space: 'Espacio exterior' };

function renderSummaryStep(stepData) {
    const d = onboardingState.userData;
    const planType = PLAN_TYPE_LABELS[d.plan_type] || d.plan_type || '—';
    const experience = EXPERIENCE_LABELS[d.experience] || d.experience || '—';
    const sessionTime = SESSION_LABELS[d.session_time] || d.session_time || '—';
    const frequency = FREQUENCY_LABELS[d.frequency] || d.frequency || '—';
    const equipment = Array.isArray(d.equipment) ? d.equipment.map(e => EQUIPMENT_LABELS[e] || e).join(', ') : (d.equipment || '—');
    return `
        <div class="summary-step">
            <h2 class="step-title">${stepData.title}</h2>
            <p class="step-subtitle">${stepData.subtitle}</p>
            <div class="summary-grid glass-card-inner" style="padding: 1.25rem; margin-top: 1rem; text-align: left;">
                <div class="summary-row"><strong>Tipo de plan:</strong> ${planType}</div>
                <div class="summary-row"><strong>Nivel:</strong> ${experience}</div>
                <div class="summary-row"><strong>Tiempo por sesión:</strong> ${sessionTime}</div>
                <div class="summary-row"><strong>Frecuencia:</strong> ${frequency}</div>
                <div class="summary-row"><strong>Equipamiento:</strong> ${equipment}</div>
            </div>
            <p class="text-secondary" style="margin-top: 1rem; font-size: 0.9rem;">Pulsa <strong>Crear mi plan</strong> para generar tu plan personalizado.</p>
        </div>
    `;
}
...
function canProceedToNext() {
    ...
    if (currentStepData.type === 'welcome' || currentStepData.type === 'summary') {
        return true;
    }
