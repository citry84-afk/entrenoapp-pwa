...
class YouTubeGallery {
    ...
    async loadVideos() {
        const noApiKey = !this.config.apiKey || this.config.apiKey === 'YOUR_YOUTUBE_API_KEY_HERE' || this.config.apiKey === '';
        try {
            if (!noApiKey && this.config.apiKey && this.config.channelId) {
                this.videos = await this.fetchVideosFromAPI();
            } else if (this.config.featuredVideos.length > 0 && !noApiKey) {
    ...
    // Playlist ID de subidas del canal (UC... -> UU...) para embeber sin API
    getUploadsPlaylistId() {
        if (!this.config.channelId) return null;
        const id = this.config.channelId.trim();
    ...
    useEmbedPlaylist() {
        const noApiKey = !this.config.apiKey || this.config.apiKey === 'YOUR_YOUTUBE_API_KEY_HERE' || this.config.apiKey === '';
        return noApiKey && this.getUploadsPlaylistId();
    }
    ...
    render() {
        ...
        const channelUrl = (this.config.channelUrl || 'https://www.youtube.com/@entrenoapp').replace(/\/?$/, '');
            ...
            container.innerHTML = `
                <div class="youtube-gallery-container">
                    <div class="youtube-gallery-header">
                        ...
                        <a href="${channelUrl}" target="_blank" rel="noopener" class="youtube-subscribe-btn">
                            Ver canal en YouTube
                        </a>
                    </div>
                    <a href="${channelVideosUrl}" target="_blank" rel="noopener" class="youtube-channel-card" style="display: block; text-decoration: none; color: inherit; border-radius: 16px; overflow: hidden; box-shadow: 0 8px 30px rgba(0,0,0,0.12); background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%); margin-top: 16px;">
                        <div style="display: flex; align-items: center; gap: 24px; padding: 32px; flex-wrap: wrap;">
                            <div style="flex: 0 0 120px; height: 120px; background: #ff0000; border-radius: 16px; display: flex; align-items: center; justify-content: center;">
                                <svg width="56" height="56" viewBox="0 0 24 24" fill="white"><path d="M8 5v14l11-7z"/></svg>
                            </div>
                            <div style="flex: 1; min-width: 200px;">
                                <h3 style="margin: 0 0 8px 0; font-size: 1.5rem; color: #fff;">Ver todos los vídeos en YouTube</h3>
                                <p style="margin: 0; color: rgba(255,255,255,0.85); font-size: 1rem;">Tutoriales, rutinas y consejos de fitness en nuestro canal. Haz clic para abrir y ver los vídeos.</p>
                                <span class="youtube-subscribe-btn" style="display: inline-block; margin-top: 16px; padding: 12px 24px;">Abrir vídeos del canal →</span>
                            </div>
                        </div>
                    </a>
                    <p style="text-align: center; margin-top: 16px; color: #666; font-size: 0.95rem;">Los vídeos se reproducen en YouTube para la mejor experiencia</p>
                </div>
            `;
            this.renderStyles();
            return true;
        ...
        // Sin API key o sin vídeos: mostrar tarjeta que enlaza al canal (el embed de playlist a veces no está disponible)
        if (this.useEmbedPlaylist() || this.videos.length === 0) {
            if (showChannelCard()) return;
        }
